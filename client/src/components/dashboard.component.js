...
export default class DashBoard extends Component {
    ...
    componentDidMount() {
        ...
        const classesdata = axios.get(`http://localhost:${port}/classroom/`, {
            headers: JSON.parse(localStorage.getItem("auth-token")),
        });

        axios
            .all([attendance, classesdata])
            .then(
    ...
    fetchAttendees(rows) {
        return rows.filter((row) => row.class_name === this.state.cQuery);
    }

    // Fetch the event from the events
    fetchClass(rows) {
        return rows.filter((row) => row.classname.indexOf(this.state.cQuery) > -1);
    }
    ...
    submitAttendance = (e) => {
        ...
        this.fetchAttendees(this.state.persons).map(person => {
            ...
            const id = person._id;
            this.fetchClass(this.state.classesd).map((eventx) => {
                                ...
                                <Col md="auto">
                                    <InputGroup className="mb-3">
                                        <Form.Control
                                            ...
                                            onChange={(e) => {
                                                this.setState({
                                                    cQuery: document.getElementById("event-input").value,
                                                });
                                            }}
                                            custom
                                        >
                                        ...
                                        <InputGroup.Append>
                                            <Button
                                                onClick={(e) => {
                                                    this.setState({
                                                        cQuery: document.getElementById("event-input")
                                                            .value,
                        ...
                        <Table striped bordered hover responsive variant={this.props.mode}>
                            <thead>
                                <tr>
                                    ...
                                    <th>Email</th>
                                    <th>Subject</th>
                                    {this.fetchClass(this.state.classesd).map((day) =>
