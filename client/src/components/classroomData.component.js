...
// Importing Logos
import ClassroomData from "../assets/images/classroomData.svg";
...
export default class EventData extends Component {
    ...
    fetchSearchResults(rows) {
      return rows
        ...
        .filter(
            (row) => row.class_name.toLowerCase().indexOf(this.state.cQuery) > -1
        );
    }
            ...
            <Jumbotron className={`${this.props.mode} py-5 px-5 m-0`}>
                <Row>
                    ...
                    <Col sm="auto" className="App-header">
                        <h1 className={this.props.mode}>Classroom Data</h1>
                    </Col>
                </Row>
                <br />
                <Form inline>
                        ...
                        <Col>
                            <InputGroup>
                                <Form.Control
                                    type="text"
                                    name="cQuery"
                                    value={this.state.cQuery}
                                    id="event-input"
                                    placeholder="Classroom"
                                    onChange={(e) => this.setState({ cQuery: e.target.value })}
                                />
                            </InputGroup>
                        </Col>
                    </Form.Row>
